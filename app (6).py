...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with bt_tab:
        ...
        st.caption("Note: The scoring model uses current fundamentals from Yahoo Finance. Technical signals (momentum, RSI, MACD) are reconstructed from historical price data. Fundamental scores represent today's values applied retroactively — a known limitation of free data sources.")

        if run_bt:
            with st.spinner(f"Running backtest for {bt_ticker.upper()} from {bt_date}…"):
                bt_res = backtest_ticker(bt_ticker.upper(), str(bt_date))
            ...
            if bt_res.get("error"):
                ...
                col2.metric("Backtest Date", bt_res["date"])
                col3.metric("Price at Date", f"${bt_res['base_price']}")
    ...
    with sc_tab:
        ...
        if run_sc:
            with st.spinner(f"Simulating {sc_ticker.upper()} from {sc_start}…"):
                df_sc, summary = run_scenario(
                    sc_ticker.upper(), str(sc_start),
            ...
            if df_sc is None:
                ...
                c4.metric("Alpha vs B&H", f"{alpha}%",
                          delta=f"{alpha}%",
                          delta_color="normal")
                c5.metric("Total Trades", summary["Trades"])
                ...
                st.download_button("⬇ Export to Excel", to_excel_bytes(df_sc),
                                   f"scenario_{sc_ticker}_{sc_start}.xlsx",
                                   mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
